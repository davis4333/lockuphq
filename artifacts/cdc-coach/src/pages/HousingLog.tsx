  ...
  return (
    ...
    >
      <section className={`${hudPanel} p-4 sm:p-5`}>
        <h2 className="text-sm font-black uppercase tracking-[0.14em] text-blue-100">
          Select official log
        </h2>
        <p className="mt-1 text-xs text-blue-200/60">
          Choose the housing unit and shift before entering the log.
        </p>
        ...
        {config && (
          <p className="mt-3 text-[11px] text-blue-300/55">
            Official source: worksheet {config.sourceSheet} · template version{" "}
        ...
        <div className="mt-4 border-t border-blue-400/20 pt-4">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            >
              <RotateCcw className="h-4 w-4" /> Resume Draft
            </button>
          </div>
        </div>
      </section>
      ...
      {!config && (
        <div
          className={`${hudPanel} mt-4 p-8 text-center text-sm text-blue-200/65`}
        >
          Select both a housing unit and shift to load the applicable official
          requirements.
        </div>
      )}
      ...
      {config && (
        <div className="mt-4 space-y-4">
          {status === "finalized" && (
            <div
              className="rounded-xl border border-emerald-400/60 bg-emerald-950/70 p-4 text-emerald-100"
              role="status"
            >
              <div className="flex items-center gap-2 font-bold">
                <CheckCircle2 className="h-5 w-5" /> Housing Log finalized
              </div>
          ...
          {issues.length > 0 && (
            <section
              ...
              className="rounded-xl border border-red-400/70 bg-red-950/70 p-4"
              role="alert"
            >
              <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.1em] text-red-100">
                <AlertTriangle className="h-5 w-5" /> Log not ready —{" "}
                {issues.length}{" "}
          ...
          <section className={`${hudPanel} p-4 sm:p-5`}>
            ...
            <div className="mt-4 space-y-3">
              {config.activities.map((item, index) => (
                <div
                  key={item.key}
                  className="rounded-lg border border-blue-400/25 bg-blue-950/25 p-3"
                >
                  <h3 className="text-xs font-bold text-blue-100">
                    {index + 1}. {item.label}
                  </h3>
                  {item.sourceNote && (
                    <p className="mt-2 rounded-md border border-amber-400/35 bg-amber-950/35 px-3 py-2 text-xs leading-relaxed text-amber-100">
          ...
          <section className={`${hudPanel} p-4 sm:p-5`}>
            ...
            <p className="mt-1 text-xs text-blue-200/60">
              Component values calculate the total automatically. Operational
              count judgments remain with staff.
            </p>
            <div className="mt-4 space-y-4">
              {config.counts.map((count) => {
                ...
                const definitions: FieldDefinition[] = [
                  ...(!count.isBeginning
                    ? [
                        {
                          key: `${prefix}.recallTime`,
                          label: "Recall time",
                          inputType: "time" as const,
                          required: true,
                        },
                        {
                          key: `${prefix}.clearTime`,
                          label: "Count clear time",
                          inputType: "time" as const,
                          required: true,
                        },
                      ]
                    : []),
                  {
                    key: `${prefix}.countTime`,
                    label: "Count time",
                    inputType: "time",
                    required: true,
                  },
                  ...count.components.map((component) => ({
                    key: `${prefix}.components.${component}`,
                    label: component,
                  ...
                  ...(count.requiresConductedBy
                    ? [
                        {
                          key: `${prefix}.conductedBy`,
                          label: count.conductedByLabel ?? "Conducted by",
                ...
                return (
                  ...
                  >
                    ...
                    {count.officialAttestation && (
                      <p className="mt-3 rounded-md border border-blue-400/25 bg-blue-950/40 px-3 py-2 text-xs leading-relaxed text-blue-100">
                        Official count attestation: {count.officialAttestation}{" "}
                        The entered initials attest to this requirement.
                      </p>
                    )}
          ...
          <section className={`${hudPanel} p-4 sm:p-5`}>
            ...
            <p className="mt-1 text-xs text-blue-200/60">
              The official form requires {config.securityCheckCount} entries for
              this unit and shift.
            </p>
          ...
          <section className={`${hudPanel} p-4 sm:p-5`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-blue-200/60">
                  If any part of a row is entered, time, activity, and initials
                  are all required.
                </p>
              </div>
              <button
            ...
            <div className="mt-4 space-y-3">
              {events.length === 0 && (
                <p className="rounded-lg border border-dashed border-blue-400/25 p-4 text-center text-xs text-blue-200/50">
              ...
              {events.map((event, index) => {
                ...
                return (
                  <div
                    key={event.id}
                    className="grid gap-3 rounded-lg border border-blue-400/25 bg-blue-950/25 p-3 sm:grid-cols-[140px_1fr_110px_auto] sm:items-end"
                  >
                    <div>
                      <label
                        className={hudLabel}
                    ...
                    <button
                      ...
                      className="mb-0.5 rounded-md border border-red-400/35 p-2.5 text-red-200 disabled:opacity-30"
                    >
          ...
          <section className={`${hudPanel} p-4 sm:p-5`}>
            ...
            <p className="mt-1 text-xs text-blue-200/60">
              The official form requires the signature areas shown below before
              finalization.
            </p>
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              {config.signatures.map((signature) => (
                <div
                  ...
                  tabIndex={-1}
                  className={`rounded-lg border bg-blue-950/25 p-3 ${errorPaths.has(`signatures.${signature.key}`) ? "border-red-400" : "border-blue-400/25"}`}
                >
                  <h3 className="mb-3 text-xs font-bold text-blue-100">
                    {signature.label}
                  </h3>
                  <SignaturePad
                    ...
                    disabled={disabled}
                    hasError={errorPaths.has(`signatures.${signature.key}`)}
                    onChange={(value) => {
          ...
          <section
            className={`${hudPanel} sticky bottom-3 z-20 flex flex-wrap items-center justify-between gap-3 p-4`}
          >
            <div className="min-w-0 text-xs text-blue-200/65">
              {notice ?? (draftId ? `Draft ID: ${draftId}` : "Not yet saved")}
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-md border border-blue-300/50 bg-blue-500/15 px-4 py-2 text-xs font-bold text-blue-100 disabled:opacity-40"
              >
              ...
              <button
                ...
                onClick={generateLog}
                disabled={disabled}
                className="inline-flex items-center gap-2 rounded-md border border-emerald-300/60 bg-emerald-500/20 px-4 py-2 text-xs font-black uppercase tracking-[0.08em] text-emerald-100 disabled:opacity-40"
              >
                <CheckCircle2 className="h-4 w-4" /> Generate Log
              </button>
            </div>
          </section>
        </div>
