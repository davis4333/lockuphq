...
- **Narrow table columns wrap because of width, not row height.** The data rows are `<w:trHeight w:hRule="exact">` (one line) with `vAlign="bottom"`; when a narrow value is slightly too wide it wraps and the row *looks* double-height. Fix per-column by threading a smaller run-props override (`<w:rPr><w:sz w:val="16"/></w:rPr>` = 8pt) and replacing break-prone chars with non-breaking equivalents (space→U+00A0, hyphen→U+2011) so the value can't break at " " or "-". Prefer this over `<w:noWrap/>` (which risks overflow/clipping in exact-height cells).
