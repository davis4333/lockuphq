...
import type { HousingLogEvent, HousingLogSignatures, HousingLogValue } from "@workspace/housing-log";

export const housingLogs = pgTable("housing_logs", {
  id: text("id").primaryKey(),
  logDate: date("log_date", { mode: "string" }).notNull(),
  shift: text("shift").notNull(),
  housingUnit: text("housing_unit").notNull(),
  templateVersion: text("template_version").notNull(),
  formValues: jsonb("form_values").$type<Record<string, HousingLogValue>>().notNull(),
