...
export function markWordInCell(cellXml: string, word: string): string {
  ...
  // Mark with strikethrough ONLY (no bold). Bold widens the glyphs, which can push
  // a tight condition/discrepancy cell (e.g. "GOOD/ SAT/ POOR") to wrap an extra
  // line and grow every row's height — that cascade is exactly the page reflow we
  // must avoid. Strikethrough (like underline) adds zero width and zero height, so
  // wrapping (and thus pagination) stays byte-for-byte identical to the blank
  // template.
