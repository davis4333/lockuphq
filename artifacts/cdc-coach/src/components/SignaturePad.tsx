...
export type SignaturePadHandle = {
  /**
   * Draws a synthetic stroke through the exact same canvas drawing calls,
   * ink-metric tracking, and isPlausibleSignatureInk gate a real
   * pointer-drawn signature goes through — used by demo seeding so a
   * generated signature is genuinely validated, not injected as a raw
   * value that skips the plausibility check. Returns false (and draws
   * nothing) if disabled.
   */
  drawSyntheticStroke: (points: readonly Point[]) => boolean;
};

const SignaturePad = forwardRef<SignaturePadHandle, SignaturePadProps>(
  function SignaturePad({ value, onChange, disabled, hasError, label }, ref) {
    ...
    const finishStroke = (canvas: HTMLCanvasElement) => {
      if (
    ...
    const start = (event: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      strokeSegment(context, point(event), true);
    };
    ...
    const draw = (event: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      const context = event.currentTarget.getContext("2d");
      if (!context) return;
      strokeSegment(context, point(event), false);
    };
    ...
    const finish = (event: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      lastPointRef.current = undefined;
      finishStroke(event.currentTarget);
    };
    ...
    useImperativeHandle(ref, () => ({
      drawSyntheticStroke: (points) => {
        if (disabled) return false;
        const canvas = canvasRef.current;
        const context = canvas?.getContext("2d");
        if (!canvas || !context || points.length === 0) return false;
        metricsRef.current = emptySignatureInkMetrics();
        lastPointRef.current = undefined;
        paintBlankCanvas(canvas);
        points.forEach((current, index) =>
          strokeSegment(context, current, index === 0),
        );
        lastPointRef.current = undefined;
        finishStroke(canvas);
        return isPlausibleSignatureInk(metricsRef.current);
      },
    }));
